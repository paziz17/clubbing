...
export default function CreateEventPage() {
  ...
  if (user?.isGuest) {
    return (
      <div className="min-h-screen bg-[#0d0d12] flex flex-col items-center justify-center px-6">
        ...
        <Link href="/auth" className="text-rose-500">התחבר</Link>
      </div>
    );
  }
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-[#0d0d12] flex flex-col items-center justify-center px-6">
        ...
        <Link href="/results" className="text-rose-500">חזרה לאירועים</Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0d0d12] px-6 py-8">
      <Link href="/results" className="text-rose-500 mb-6 inline-block">← חזרה</Link>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <input
          ...
          required
          className="w-full px-4 py-3 bg-[#16161d] border border-zinc-700 rounded-xl text-white placeholder-zinc-500"
        />
        <div className="flex gap-2">
          <input
            ...
            required
            className="flex-1 px-4 py-3 bg-[#16161d] border border-zinc-700 rounded-xl text-white"
          />
          <input
            type="time"
            value={time}
            onChange={(e) => setTime(e.target.value)}
            className="w-32 px-4 py-3 bg-[#16161d] border border-zinc-700 rounded-xl text-white"
          />
        </div>
        <input
          type="text"
        ...
        <textarea
          ...
          rows={3}
          className="w-full px-4 py-3 bg-[#16161d] border border-zinc-700 rounded-xl text-white placeholder-zinc-500 resize-none"
        />
        <input
          type="url"
